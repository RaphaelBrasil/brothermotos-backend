...
async function run() {
	try {
		...
		app.post("/email", async (req, res) => {
			...
			const msg = {
				to: user,
				from: "rt.raphael.oliveira@gmail.com", // Apenas para teste
				subject: "Alteração na tarefa",
				html: `Oi, sua tarefa: "${req.body.text}" foi alterada!</strong>`
			};
			...
			sgMail
				...
				.catch((error) => {
					...
					return res
						.status(400)
						.send(`Não foi possível enviar o email: ${error}\n`);
				});
		});
